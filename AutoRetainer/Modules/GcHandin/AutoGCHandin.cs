...
internal static unsafe class AutoGCHandin
{
    ...
    // ── 繳交流程狀態機 ────────────────────────────────────────────────────
    // 每一步的閘門都是「條件成不成立」，沒有任何固定延遲；不成立就下一幀再試。
    // 按下交付、獎勵視窗一關掉，就主動對 AgentGrandCompanySupply 送出重選分頁事件逼它當場重建清單（GCSupplyRefresh），不再空等遊戲自己來。
    // 逾時只是保險絲：任何一步的假設不成立時，退回「等遊戲自己重建」的舊行為重新掃描，不會卡死也不會崩。
    ...
    // ── 「等刷新閘門」那半秒是誰欠的 ──────────────────────────────────────
    // 獎勵視窗一消失，清單視窗就**不存在**，代理人的
    // 清單陣列與 AddonId 也都是 0。也就是說刷新事件在那一刻
    ...
    /// <summary>
    /// 這一趟因為軍票已達上限而中斷過幾次。
    /// 🔑 軍票續購（<c>C.AutoGCContinuation</c>）開著時這是**正常節奏**的一部分。
