...
public class GeneralSettings : InventoryManagemenrBase
{
    ...
    private GeneralSettings()
    {
        Builder = InventoryCleanupCommon.CreateCleanupHeaderBuilder()
            .Section(Name)
            .Checkbox($"Auto-open venture coffers", () => ref InventoryCleanupCommon.SelectedPlan.IMEnableCofferAutoOpen, Loc.T("Multi Mode only. Before logging out, all coffers will be opened unless your inventory space is too low."))
            .Checkbox($"Enable selling items to retainer", () => ref InventoryCleanupCommon.SelectedPlan.IMEnableAutoVendor, Loc.T("When AutoRetainer checks resents retainers to ventures, items will be sold according to Inventory Cleanup plan."))
            .Checkbox($"Enable selling items to housing NPC", () => ref InventoryCleanupCommon.SelectedPlan.IMEnableNpcSell, Loc.T("When AutoRetainer enters a house, items will be sold according to the Inventory Cleanup plan. A housing vendor that supports item selling must be placed near the house entrance (not the workshop entrance)—you should be able to interact with the NPC immediately after entering."))
            .Indent()
