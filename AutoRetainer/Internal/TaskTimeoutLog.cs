...
/// 唯一帶任務名稱的那一行（<c>→→Task timed out {Name}@{Location}</c>）被 <c>ShowDebug</c> 閘住，
/// 而本外掛的 <c>ShowDebug</c> 跟隨 <c>/autoretainer debug</c>，預設是關的。
/// </para>
/// <para>
/// ⇒ 雇員存取金幣、軍票交付、潛水艇排程這些鏈上任何一步卡住而
/// <c>AbortOnTimeout</c> 清掉整條佇列時，日誌上查不出是哪一步。
/// </para>
/// <para>
...
public static class TaskTimeoutLog
{
    ...
    /// 事後對傳進去的那個物件指派完全沒有效果。
    /// <para>
    /// ⚠️ 也因此，<c>SyncTaskManagerDebugOutput()</c> 那種「事後改 <c>DefaultConfiguration.ShowDebug</c>」
    /// 的寫法是對的（改的是同一份），這裡沿用同一個位置。
    /// </para>
    /// <para>
